...
export function RoundReplayPage({ demoId, initialRound, onBack }: Props) {
  ...
  useEffect(() => {
    if (!proMappingEnabled || !data || playing) return undefined;

    const anchorTick = curTick || data.freeze_end_tick;
    let active = true;
    const handle = window.setTimeout(() => {
      setMappingLoading(true);
      setMappingError(null);
      fetch(`/api/similarity-map/${encodeURIComponent(demoId)}/${roundNum}/${anchorTick}`)
        .then((r) => { if (!r.ok) throw new Error(`${r.status}`); return r.json(); })
  ...
  }, [curTick, data, demoId, playing, proMappingEnabled, roundNum]);
  ...
  const proTickIdx = useMemo(() => {
    if (!proData || !mapping?.best_match) return 0;
    const targetTick = mapping.best_match.anchor_tick + (curTick - mapping.query.anchor_tick);
    return findNearestTickIndex(proData.tick_list, targetTick);
  }, [curTick, mapping, proData]);
  ...
  const mappingStatus = !proMappingEnabled
    ? "User-only view"
    : mappingLoading
      ? "Mapping current moment to pro play..."
      : bestMatch
        ? `Top match · ${Math.round(bestMatch.score * 100)}%`
        : "No pro mapping available";

  const showProPanel = proMappingEnabled;
  const userTimelineNote = mapping?.query
    ? `${phaseLabel(mapping.query.phase)}${mapping.query.site ? ` · site ${mapping.query.site.toUpperCase()}` : ""}`
    : null;
        ...
        {showProPanel && (
          <div style={styles.panel}>
            ...
            <div ref={proContainerRef} style={styles.canvasWrap}>
              {mappingLoading && <div style={styles.overlay}>Mapping current moment…</div>}
              {mappingError && <div style={styles.overlay}>Mapping error: {mappingError}</div>}
              {!mappingLoading && !mappingError && !bestMatch && (
                <div style={styles.overlay}>No mapped pro event for this moment yet.</div>
              )}
      ...
      <div style={styles.hud}>
        ...
        <div style={styles.timelineStack}>
          <TimelineBar
            ...
            highlightStart={mapping?.query.start_tick}
            highlightEnd={mapping?.query.end_tick}
            accentColor="#6882d2"
        ...
        <div style={styles.hint}>
          ...
          {showProPanel ? " · pro panel follows the mapped event window" : ""}
        </div>
      </div>
    </div>
  );
