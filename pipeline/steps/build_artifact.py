...
def _player_nav_sequences(
    ...
    """Compute per-player nav-area sequences sampled at fixed intervals."""
    from backend.nav_matching import lookup_nav_area_id, collapse_route_steps

    if round_ticks.height == 0 or "tick" not in round_ticks.columns or "steamid" not in round_ticks.columns:
        return {}

    freeze_end = int(round_meta.get("freeze_end_tick") or 0)
    end_tick = int(round_meta.get("official_end_tick") or round_meta.get("end_tick") or freeze_end)
    bomb_plant_tick = round_meta.get("bomb_plant_tick")
    step_ticks = max(1, int(round(sample_step_sec * TICK_RATE)))

    all_ticks = sorted(round_ticks["tick"].unique().to_list())
    if not all_ticks:
        return {}

    target_ticks = list(range(freeze_end, end_tick + 1, step_ticks))
    ...
    for target in target_ticks:
        idx = bisect.bisect_left(all_ticks, target)
        candidate = all_ticks[idx] if idx < len(all_ticks) else all_ticks[-1]
        if candidate not in seen:
            seen.add(candidate)
            snapped.append(candidate)

    tick_frames: dict[int, pl.DataFrame] = {}
    for tick_val in snapped:
        frame = round_ticks.filter(pl.col("tick") == tick_val)
        if frame.height > 0:
            tick_frames[tick_val] = frame

    result: dict[str, dict] = {}
    player_meta = (
        round_ticks.select(["steamid", "name", "side"])
        .unique(subset=["steamid"])
        .to_dicts()
    )
    winner = str(round_meta.get("winner") or "").lower()
    round_end_sec = max(0.0, (end_tick - freeze_end) / TICK_RATE)

    for player in player_meta:
        steamid_str = str(player["steamid"])
        ...
        for tick in snapped:
            frame = tick_frames.get(tick)
            if frame is None:
                continue
            t_sec = (tick - freeze_end) / TICK_RATE
            player_row = frame.filter(pl.col("steamid") == player["steamid"])
            if player_row.height == 0:
                continue

            alive = int(player_row["health"][0] or 0) > 0 if "health" in player_row.columns else True
            area_id: int | None = None
            if alive and "X" in player_row.columns:
                area_id = lookup_nav_area_id(
                    map_name,
                    float(player_row["X"][0] or 0),
                    float(player_row["Y"][0] or 0),
                    float(player_row["Z"][0] or 0),
                )

            bomb_planted = (bomb_plant_tick is not None) and (tick >= int(bomb_plant_tick))

            support = 0
            if alive and "health" in frame.columns and "X" in frame.columns:
                px, py = float(player_row["X"][0] or 0), float(player_row["Y"][0] or 0)
                teammates = frame.filter(
                    (pl.col("side").str.to_lowercase() == side)
                    & (pl.col("health") > 0)
                    & (pl.col("steamid") != player["steamid"])
                )
                for row in teammates.iter_rows(named=True):
                    dx = float(row.get("X") or 0) - px
                    dy = float(row.get("Y") or 0) - py
                    if (dx * dx + dy * dy) ** 0.5 <= teammate_radius:
                        support += 1

            enemy = 0
            if alive and "health" in frame.columns and "X" in frame.columns:
                enemies = frame.filter(
                    (pl.col("side").str.to_lowercase() != side)
                    & (pl.col("health") > 0)
                )
                for row in enemies.iter_rows(named=True):
                    dx = float(row.get("X") or 0) - px
                    dy = float(row.get("Y") or 0) - py
                    if (dx * dx + dy * dy) ** 0.5 <= enemy_radius:
                        enemy += 1

            nearby_deaths = 0
            if previous_frame is not None and "health" in previous_frame.columns and "X" in previous_frame.columns:
                previous_player = previous_frame.filter(pl.col("steamid") == player["steamid"])
                if previous_player.height > 0 and int(previous_player["health"][0] or 0) > 0:
                    px_prev = float(previous_player["X"][0] or 0)
                    py_prev = float(previous_player["Y"][0] or 0)
                    previous_teammates = previous_frame.filter(
                        (pl.col("side").str.to_lowercase() == side)
                        & (pl.col("health") > 0)
                        & (pl.col("steamid") != player["steamid"])
                    )
                    for teammate in previous_teammates.iter_rows(named=True):
                        dx = float(teammate.get("X") or 0) - px_prev
                        dy = float(teammate.get("Y") or 0) - py_prev
                        if (dx * dx + dy * dy) ** 0.5 > teammate_radius:
                            continue
                        current_teammate = frame.filter(pl.col("steamid") == teammate["steamid"])
                        if current_teammate.height > 0 and int(current_teammate["health"][0] or 0) <= 0:
                            nearby_deaths += 1

            sample_times.append(round(t_sec, 3))
            sample_ticks.append(int(tick))
            area_ids.append(area_id)
            support_counts.append(support)
            enemy_counts.append(enemy)
            nearby_teammate_deaths.append(nearby_deaths)
            bomb_flags.append(bomb_planted)
        ...
        route_steps = collapse_route_steps(sample_times, area_ids, support_counts, enemy_counts, bomb_flags)
        if not route_steps:
            continue

        death_sec: float | None = None
        seen_alive = False
        for t, alive in zip(sample_times, alive_flags):
        ...
        result[steamid_str] = {
            ...
            "local_balance": [
                ...
                for support, enemy in zip(support_counts, enemy_counts)
            ],
            ...
            "round_winner": winner,
            "round_reason": str(round_meta.get("reason") or ""),
            "round_end_sec": round(round_end_sec, 3),
            "round_num": int(round_meta.get("round_num") or 0),
            "steamid": steamid_str,
            "map_name": map_name,
            "side": side,
...
def build_match_artifact(
    *,
    ...
    for round_row in rounds.iter_rows(named=True):
        ...
        nav_seqs = _player_nav_sequences(round_ticks, round_meta, map_name)
            ...
            "teams": {
                side: {
                    ...
                    "nav_trace": {
                        ...
                        "player_nav_sequences": {
                            sid: seq for sid, seq in nav_seqs.items()
                            if seq.get("side") == side
                        },
                    },
                }
                for side in _SIDES
